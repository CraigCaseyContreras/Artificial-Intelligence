...
class GeneticAlgorithm:
	...
	def pass_time(self, population, generations):
      ...
      if len(optimal_dists) < 2:
      	pass
      else: 
      	indx1 = len(optimal_dists)
      	indx2 = indx1 - 1
      	diff = optimal_dists[indx1] - optimal_dists[indx2]
      	if diff < np.std(optimal_dists):
      		print(optimal_routes[indx1], 'OPTIMAL ROUTE 1')
      		print(optimal_routes[indx2], 'OPTIMAL ROUTE 2')
      		print('\n')
				print(optimal_dists[indx1], 'OPTIMAL DISTS 1')
				print(optimal_dists[indx2], 'OPTIMAL DISTS 2')
				print('\n')
