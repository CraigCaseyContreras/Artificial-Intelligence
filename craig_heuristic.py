...
class State(object):
    ...
    def getIndices(self, pair_points):
        ...
        indices = list(
            zip(individual_points, individual_points[1:] + individual_points[:1]))
        del indices[1::2]
        return indices
    ...
    def CreateChildren(self, board):
        ...
        for i in players:
            print(i)
            # List of every coordinate that is 0
            have_sx = np.where(board == i)
...
class HillSolver:
    ...
    def solve(self):
        initialState = State(self.start, 0, self.start,
                             self.goal)  # 0 because not a parent
        # returns [3,4,2] - the distances of 1,2,3 respectively
        distances, players = initialState.GetDist(self.start)
