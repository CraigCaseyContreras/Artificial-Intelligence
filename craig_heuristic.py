...
class State(object):
    ...
    def GetDistPerm(self, perm):
        ...
        # fixed the for loop below to keep the nested structure for distances
        for i in range(len(perm)):
            ...
            for k in range(len(unique_val)):
                ...
                distance = y_val + x_val
                # appends distances to list
                permdistlist.append(distance)
            dist_list.append(permdistlist)
        return dist_list

    # Need to figure out this. All it does it makes it starts with 3
    def eval_function(self, perm):
        permlist = self.GetDistPerm(perm)
        addeddistlist = []
        # very basic evaluation function will add up distances and return the lowest one
        for i in permlist:
            addeddist = 0
            for j in i:
                addeddist += j
            addeddistlist.append(addeddist)
        print(addeddistlist, "gotheregothere")
    ...
    def neighbors(self, index):
        ...
        for relative_index in product((-1, 0, 1), repeat=N):
            if not all(i == 0 for i in relative_index):
                everyone = list(i + i_rel for i,
                                i_rel in zip(index, relative_index))
                yield everyone
    ...
    def CreateChildren(self):
        ...
        for i in players:
            print(i)
            # List of every coordinate that is 0,1,2,3,....
            have_sx = np.where(self.start == i)
...
class HillSolver:  # EVAL FUNCTION SHOULD BE IN THIS CLASS BECAUSE IT IS DIFFERENT FOR EVERY ALGORITHM!
    def __init__(self, start, goal):
        # this self.path should include the intial board and the children it chooses to go to.
        self.path = []
        self.start = start
        self.goal = goal
