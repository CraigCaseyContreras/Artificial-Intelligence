    ...
    def __init__(self, value, parent, start=0, goal=0):
        ...
        if parent:  # Meaning if the parent is plugged in
            self.path = parent.path[
                        :]  # copy parent path to our path, so if make changes wont make changes to parent path
...
class AStar_Solver:
    ...
    def Solve(self):
        ...
        while (not self.path and self.priorityQueue.qsize()):
            ...
            closestChildTuple = self.priorityQueue.get()
            closestChild = closestChildTuple[1]
